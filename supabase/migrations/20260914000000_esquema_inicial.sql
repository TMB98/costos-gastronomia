...
- **Logging + monitoreo de errores con `@sentry/react`**: módulo nuevo
  ...
  usuario, no fallas de la app, reportarlos sería solo ruido.
  **Confirmado funcionando en producción (14/09/2026)**: proyecto
  "la-nuna" en sentry.io (org tomas-botindari), DSN cargado como secret
  `VITE_SENTRY_DSN`. Prueba real: un error tirado desde la consola del
  navegador en la app publicada apareció en Sentry → Issues en menos de
  1 minuto, con el release correcto taggeado.
- **Banner de "sin conexión"** (v1.18): hook `src/lib/useEnLinea.js`
  (`navigator.onLine` + eventos `online`/`offline`), banner fijo arriba de
  todo en `App.jsx` cuando se corta la conexión. No se replicó en login ni
  en la pantalla de carga — ahí el propio mensaje de error ya cumple ese
  rol.
- **Flujo con Pull Request + protección de rama**: **activo y confirmado**
  (14/09/2026). Regla clásica en GitHub (Settings → Branches → `main`):
  requiere PR para mergear con 0 aprobaciones obligatorias (Tomi trabaja
  solo — en la UI de GitHub esto se logra destildando el checkbox "Require
  approvals" entero, no hay una opción "0" en el desplegable), + requiere
  que el check `build` pase. `deploy.yml` corre tests+build también en
  PRs, pero el `deploy` real solo corre en push a `main` (nunca en un PR
  sin mergear). GitHub ahora le ofrece a Tomi "Create a new branch and
  start a pull request" en vez de dejarlo commitear directo — ya probado
  de punta a punta con el PR #1.
- **`supabase/migrations/`**: creada, con `20260914000000_esquema_inicial.sql`
  como línea base — una FOTO fiel del esquema real de producción (12
  tablas, funciones RPC, RLS, políticas), sacada con
  `.github/workflows/exportar-esquema.yml` (workflow nuevo, de solo
  lectura, disparo manual — usa el mismo secret `SUPABASE_DB_URL` que el
  backup). A partir de esta foto, todo cambio de esquema nuevo va en un
  archivo nuevo en esa carpeta, nunca editando este.
  ⚠️ **Hallazgo al sacar la foto**: la tabla `datos_app` (del modelo viejo
  de JSON único) y sus políticas de RLS **siguen existiendo en
  producción**, sin uso real — confirmado, nada del código actual la
  toca. Queda anotada en el archivo de migración, pero NO se borró
  todavía (pendiente chico, con backup fresco antes de tocarla).
- Restore automatizado (workflow de GitHub Actions): **decidido no
  construirlo todavía** — se prueba primero contra el proyecto de
  desarrollo de Supabase (siguiente punto pendiente) antes de confiar en
  algo así para producción.
...
1. Segundo proyecto de Supabase (gratis) para desarrollo, sin datos
   reales — aplicarle la migración inicial ahí para tener un lugar seguro
   donde probar cambios de esquema y el restore.
2. Probar el restore de verdad contra ese proyecto de desarrollo.
3. Pendiente chico: borrar la tabla `datos_app` (con backup fresco antes).
4. Opcional/baja prioridad: ping de disponibilidad (ej. UptimeRobot) a la
   URL de producción.
